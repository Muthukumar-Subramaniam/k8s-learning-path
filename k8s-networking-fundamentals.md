...
│  └──────┬──────┘         └───────┬────────┘                                    │
│         │                        │                                              │
│         │ Programs               │ Manages Routes                               │
│         ▼                        ▼                                              │
│  ┌─────────────────────────────────────────────────────────┐                   │
│  │           Linux Kernel Network Stack                    │                   │
│  │  ┌─────────┐ ┌────────┐ ┌────────┐ ┌──────┐ ┌──────┐  │                   │
│  │  │iptables │ │routing │ │ cali*  │ │ veth │ │ eth0 │  │                   │
│  │  │  rules  │ │ tables │ │ ifaces │ │pairs │ │      │  │                   │
│  │  └─────────┘ └────────┘ └───┬────┘ └──┬───┘ └──────┘  │                   │
│  └────────────────────────────────┼─────────┼──────────────┘                   │
│                                   │         │                                  │
│                                   ▼         ▼                                  │
...
│                                                                                │
│  Similar structure: kubelet, kube-proxy, calico-node, Container Runtime       │
│  Calico dynamically allocates IP blocks (/26) from pool as needed             │
│  Pods with IPs: 10.8.x.x (allocated by Calico IPAM)                           │
...
│                                                                                │
│  🌐 Pod Network (10.8.0.0/16) - Calico IPPool                                 │
│     Calico manages IP allocation using /26 blocks (62 IPs per block)          │
│     Dynamic allocation: blocks assigned on-demand as pods are created         │
│     No fixed per-node CIDR - flexible multi-block allocation                  │
│                                                                                │
